...
app.get("/matrix", (req, res, next) => {
	...
	//get symbols from params
	 ...
	 createGraphForUniverseInDb(stock_universe).then(function(result){
		...
		for (let i = 0; i < numberOfBatches; i++) {
		  ...
		  fetch(url).then(response => response.json()).then(json => {
				...
				if(i + 1 === numberOfBatches){
					...
					createStockNodesInDb(stock_universe, chartData);
					resolve(response);
					/*
...
function createStockNodesInDb(stock_universe, chartData){
	...
	stock_universe.forEach(symbol => {
		...
		db.cypher({
			query: 'CREATE ({symbolParam}:stock { company: {companyParam}, industry: {industryParam}, sector: {sectorParam})',
			params: {
				symbolParam: symbol, 
				companyParam: chartData[symbol].company.companyName, 
				industryParam: chartData[symbol].company.industry, 
				sectorParam: chartData[symbol].company.sector
			},
